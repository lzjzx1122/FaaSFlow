...
bandwidth = 10000000 / 8 ## 10Mbit/s

def parseFunction(data, name):
    parameters = dataAtIndex(data, 'parameters')
    source = dataAtIndex(data, 'source')
    next = dataAtIndex(data, 'next')
    # nextDis = dataAtIndex(data, 'time') ## for grouping_examples cases
    ...
    ## for benchmark cases
    nextDis = []
    files = dataAtIndex(data, 'files')
    send_time = 0
    if files is not None:
        send_byte = 0
        for file in files:
            if dataAtIndex(file, 'link'):
                send_byte += dataAtIndex(file, 'size')
        send_time = send_byte / bandwidth
    for _ in next:
        nextDis.append(send_time)
    runtime = dataAtIndex(data, 'runtime')
    if runtime is None:
        runtime = 0

    res = Function(name, next, nextDis, source, parameters, runtime)
    objectMap[name] = res
    return res

def parseSwitch(data, name):
    next = dataAtIndex(data, 'next')
    nextDis = dataAtIndex(data, 'nextDis')
    if next is None:
        next = []
        nextDis = []
    elif type(next) is str:
    ...
    res = Switch(name, next, nextDis, choices_str)
    objectMap[name] = res
    return res
