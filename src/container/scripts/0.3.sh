...
download_limit=120mbit
upload_limit=120mbit


# Filter options for limiting the intended interface.
PRI_FILTER="$TC filter add dev $interface protocol ip parent 1: prio 1 u32"
FILTER="$TC filter add dev $interface protocol ip parent 1: prio 10 u32"
...
#    sleep 1
    ...
    $TC class add dev $interface parent 1:1 classid 1:10 htb rate $interface_speed burst 15k
    ...
    $PRI_FILTER match ip dst 172.17.0.1 flowid 1:10
#    $PRI_FILTER match ip dst 172.17.0.2 flowid 1:10
    $FILTER match ip dst 0.0.0.0/0 flowid 1:20
