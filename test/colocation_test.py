...
def end_loop(idx, workflow_name, parallel, duration):
    while time.time() - test_start < pre_time + duration:
        post_request(f'{workflow_name}_request_' + str(idx).rjust(4, '0'), workflow_name)
        idx += parallel
...
workflow_names = ['recognizer', 'video', 'wordcount']


def test_to_one(loop_cnt, duration):
    ...
    ids = {}
    latencies = {x: [] for x in workflow_names}
    print(f'firing colocation with loop {loop_cnt} for {duration} s')
    global test_start
    ...
    #     t.join()
    for workflow_name in workflow_names:
        print(f'---latency results of {workflow_name}---')
        now_latencies = latencies[workflow_name]
        print('total requests count:', len(now_latencies))
    ...
    if not os.path.exists('result'):
        os.mkdir('result')
    suffix = 'colocation_' + str(loop_cnt) + '_' + str(duration) + f'_({input_args})'
    filepath = os.path.join('result', nowtime + '_' + suffix + '.json')
    with open(filepath, 'w') as f:
...
def test_to_all():
    ...
    #     'wordcount': {23: 5},
    #     }
    target_loop = {1: 10, 2: 10, 4: 10}
    # target_workflow = {'recognizer': {1: 10, 2: 10, 4: 10, 5: 10, 6: 10, 7: 10, 8: 10},
    #                    'video': {1: 10, 2: 10, 4: 10, 8: 10, 10: 10, 12: 10, 14: 10, 15: 10, 16: 10},
    #                    'wordcount': {1: 5, 2: 5, 4: 5, 8: 5, 10: 5, 12: 5, 14: 5, 16: 5, 17: 5, 18: 5}}
    # target_workflow = {
    #                    'wordcount': {1: 5, 2: 5, 4: 5, 8: 5, 10: 5, 12: 5, 14: 5, 16: 5}}
    for loop_cnt in target_loop:
        test_to_one(loop_cnt, 60 * target_loop[loop_cnt])
