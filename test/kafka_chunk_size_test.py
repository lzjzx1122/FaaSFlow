...
def get_use_container_log(suffix):
    ...
    for function_name in cnt:
        ...
        print()
        if 'split' in function_name:
            trans_end = avg[function_name][1] / cnt[function_name]
        if 'upload' in function_name:
            trans_start = avg[function_name][2] / cnt[function_name]
    # print('Redis_KB-s:', format(total_redis / len(latencies), '.0f'))
    print('trans_duration:', format(trans_end - trans_start, '.3f'))
...
def test_to_one(workflow_name, chunk_size, duration):
    repo.clear_couchdb_workflow_latency()
    repo.clear_couchdb_results()
    repo.save_kafka_config(chunk_size)
    ...
    ids = {}
    latencies = []
    print(f'firing KAFKA_CHUNK_SIZE with {chunk_size} for {duration} s')
    global test_start
    ...
    suffix = 'KAFKA_CHUNK_SIZE' + '_' + str(chunk_size) + '_' + str(duration) + f'_({input_args})'
...
def test_to_all():
    print(input_args)
    kafka_chunk_size_KB = [16, 32, 64, 128, 256, 384, 512, 640, 768, 896]
    kafka_chunk_size_KB = [16, 32, 384, 640, 896]
    kafka_chunk_size_KB.reverse()
    for now_chunk_size in kafka_chunk_size_KB:
        test_to_one('video', now_chunk_size * 1024, 60 * 5)
