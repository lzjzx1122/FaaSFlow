...
command = 'ffmpeg -i '+ input_filepath + ' -c copy -f segment -segment_time ' + segment_time_seconds + " -reset_timestamps 1 " + video_proc_dir + "/split_"+ shortname + '_piece_%02d' + extension
...
for filename in os.listdir(video_proc_dir):
    if filename.startswith('split_' + shortname):
        ...
        with open(filekey, 'rb') as f: splited_video = f.read()
