...
def preview_generate(video_name):
    ...
    preview_name =  shortname + '_preview.gif'
...
video_name = os.path.join(ENV_WORKDIR, 'test.mp4')
shutil.copy('/proxy/test.mp4', video_name)
...
store.post('video_name', 'test.mp4')
store.post('user_name', 'Barrin')
store.post('segment_time', 5)
