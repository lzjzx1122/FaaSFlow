...
def main():
    ...
    transcoded_input_path = os.path.join('work', 'split_output')
    if not os.path.exists(transcoded_input_path):
        os.mkdir(transcoded_input_path)
    for k in split_keys:
        fileDir, filename, shortname, extension = get_fileNameExt(k)
        transcoded_input_filename = 'transcoded_%s.%s' % (shortname, target_type)
    ...
    segs_filepath = os.path.join('work', segs_filename)
    ...
    merged_filepath = os.path.join('work', merged_filename)
