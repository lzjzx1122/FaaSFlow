...
def analyze_overall_workflow():
    ...
    repo.mem_clearall()
    ids = [str(uuid.uuid4()) for i in range(20)]
    imgs = ['01.jpg', '02.jpg']
    for i in range(20):
        id = ids[i]
        print('----preparing basic input for request ', id, '----')
        prepare_basic_input.main(id, imgs[i%2], 'ziliuziliu')
    e_total = 0
    n_total = 0
    overall_total = 0
    cnt = 0
    for id in ids:
        ...
        cnt = cnt + 1
    print('----overall time: ', overall_total / 16, ' edge_time: ', e_total / 16, ' node_time: ', n_total / 16, '----')
analyze_overall_workflow()

# edge_time, node_time = analyze_each_function('5678')
